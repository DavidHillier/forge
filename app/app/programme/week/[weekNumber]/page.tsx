...
export default async function WeekDetailPage({ params }: { params: Promise<{ weekNumber: string }> }) {
  ...
  const circuitWorkouts = week.workouts.filter((workout) => workout.type === "Dumbbell Circuit");
  const circuitDayNumbers = new Set(circuitWorkouts.map((workout) => workout.dayNumber));
  const completedCircuitDays = new Set(
    completions
      .filter((completion) => circuitDayNumbers.has(completion.workout.dayNumber))
      .map((completion) => completion.workout.dayNumber),
  );
    ...
    <AppShell active="Programme">
      <div className="grid gap-6">
        ...
        <Card className="grid gap-2 sm:grid-cols-3">
          ...
          <p><span className="text-sm text-[#6B756F]">Circuits complete</span><br />{completedCircuitDays.size} / {circuitWorkouts.length}</p>
          <p><span className="text-sm text-[#6B756F]">Circuit count</span><br />{week.circuitCount ?? "Prescribed progression"}</p>
        </Card>
        <div className="grid gap-3">
          {circuitWorkouts.map((workout) => (
            <Card key={workout.id} className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center">
              <div className="flex items-center gap-3">
                {completedCircuitDays.has(workout.dayNumber) ? <CheckCircle2 className="text-[#1E6F4B]" /> : <Circle className="text-[#B9903D]" />}
                <div>
